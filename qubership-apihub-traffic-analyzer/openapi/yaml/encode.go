...
import (
	...
	"github.com/Netcracker/qubership-apihub-traffic-analyzer/qubership-apihub-traffic-analyzer/openapi/yaml/internal/errors"
	"github.com/Netcracker/qubership-apihub-traffic-analyzer/qubership-apihub-traffic-analyzer/openapi/yaml/parser"
	"github.com/Netcracker/qubership-apihub-traffic-analyzer/qubership-apihub-traffic-analyzer/openapi/yaml/printer"
...
func (e *Encoder) canEncodeByMarshaller(v reflect.Value) bool {
	if !v.CanInterface() {
		return false
	}
	iface := v.Interface()
	...
	case encoding.TextMarshaler:
		return true
	case jsonMarshaller:
		return e.useJSONMarshaller
	}
	return false
}

func (e *Encoder) encodeByMarshaller(ctx context.Context, v reflect.Value, column int) (ast.Node, error) {
	iface := v.Interface()

	if marshaller, ok := iface.(BytesMarshalerContext); ok {
		doc, err := marshaller.MarshalYAML(ctx)
		if err != nil {
	...
	if marshaller, ok := iface.(BytesMarshaler); ok {
		doc, err := marshaller.MarshalYAML()
		if err != nil {
			return nil, errors.Wrapf(err, "failed to MarshalYAML")
		}
		node, err := e.encodeDocument(doc)
	...
	if marshaller, ok := iface.(InterfaceMarshalerContext); ok {
		marshalV, err := marshaller.MarshalYAML(ctx)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to MarshalYAML")
		}
	...
	if marshaller, ok := iface.(InterfaceMarshaler); ok {
		marshalV, err := marshaller.MarshalYAML()
		if err != nil {
			return nil, errors.Wrapf(err, "failed to MarshalYAML")
		}
		return e.encodeValue(ctx, reflect.ValueOf(marshalV), column)
	...
	if marshaller, ok := iface.(encoding.TextMarshaler); ok {
		doc, err := marshaller.MarshalText()
		if err != nil {
	...
	if e.useJSONMarshaller {
		if marshaller, ok := iface.(jsonMarshaller); ok {
...
func (e *Encoder) encodeValue(ctx context.Context, v reflect.Value, column int) (ast.Node, error) {
	...
	if e.canEncodeByMarshaller(v) {
		node, err := e.encodeByMarshaller(ctx, v, column)
		if err != nil {
	...
	return nil, nil // might be unreachable
}
